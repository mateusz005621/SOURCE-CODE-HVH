...
bool Chams::CreateMaterials( ) {
	...
	m_matFlat = g_pMaterialSystem->FindMaterial( ( "debug/debugdrawflat" ), nullptr );
	m_matRegular = g_pMaterialSystem->FindMaterial( XorStr( "pdr_shine" ), TEXTURE_GROUP_MODEL );
...
void Chams::OverrideMaterial( bool xqz, const Color_f& color, IMaterial* pOverrideMaterial ) {
	...
	g_pStudioRender->m_pForcedMaterial = material;
	g_pStudioRender->m_nForcedMaterialType = 0;
}
...
bool Chams::HandlePlayerChams( C_CSPlayer* player, std::function<void( )> oFn ) {
	...
	// note - maxwell; this local check has to be here or the cheat blows up without a call stack/crash dump ;^)
	if( player->EntIndex() != g_pEngine->GetLocalPlayer() ) {
		if( g_PlayerList.GetSettings( player->GetSteamID() ).m_bDisableVisuals ) {
			return false;
		}
	}

	if( g_Misc.m_flThirdpersonTransparency != 1.f && player->EntIndex( ) == pLocal->EntIndex( ) ) {
		g_pRenderView->SetBlend( g_Misc.m_flThirdpersonTransparency );
		oFn( );
		return true;
	}
