...
bool GUI::Controls::Listbox( const std::string& id, std::vector<std::string> elements, int* option, bool bSearchBar, int iSizeInElements ) {
	...
	bool bHoveredSearch = g_InputSystem.IsInBox( DrawPos - Vector2D( 0, 18 ), Vector2D( iWidth, 19 ) ) && g_InputSystem.IsInBox( ctx->ParentPos, ctx->ParentSize ) && bSearchBar && !GUI::ctx->disabled;
	bool bHovered = g_InputSystem.IsInBox( DrawPos, Vector2D( iWidth, kurwa * iSizeInElements ) ) && g_InputSystem.IsInBox( ctx->ParentPos, ctx->ParentSize ) && !GUI::ctx->disabled;

	Render::DirectX::rect_fill( DrawPos, Vector2D( iWidth, kurwa * iSizeInElements ), ctx->disabled ? Color( 23, 23, 23 ) : Color( 35, 35, 35 ) );
	...
	if( elements.size( ) )
		for( int i = 0; i < elements.size( ); i++ ) {
			...
			if( bHovered && bIsInBounds && !GUI::ctx->disabled ) {
