...
std::vector<std::string> ConfigManager::GetConfigs( ) {
	...
	if( created_cfg ) {
		...
		std::wstring str = full_path.wstring( ) + XorStr( L"\\kaaba" );
	...
	std::string config_extension = XorStr( ".kab" );
	std::vector<std::string> names;

	WIN32_FIND_DATAA find_data;
	HANDLE preset_file = FindFirstFileA( ( XorStr( "kaaba\\cfg\\*" ) + config_extension ).c_str( ), &find_data );
	...
	if( preset_file != INVALID_HANDLE_VALUE ) {
		do {
			...
			// no.
			if( s == XorStr( "global_data.kab" ) )
				continue;

			int pos = s.find( XorStr( ".kab" ) );
...
void ConfigManager::LoadConfig( std::string configname, bool load_user_data ) {
	std::ifstream input_file = std::ifstream( ( XorStr( "kaaba\\cfg\\" ) + configname + XorStr( ".kab" ) ).c_str( ) );
	if( !input_file.good( ) )
		return;
...
void ConfigManager::SaveConfig( std::string configname, bool load_user_data ) {
	std::ofstream o( ( XorStr( "kaaba\\cfg\\" ) + configname + XorStr( ".kab" ) ).c_str( ) );
	if( !o.is_open( ) )
		return;
	...
	o.clear( );
	auto str = base64::encode( ( std::string( XorStr( "[kab] - " ) ).append( g_Vars.m_json.dump( -1, '~', true ) ) ).c_str( ) );
	o << str;
	o.close( );
...
void ConfigManager::RemoveConfig( std::string configname ) {
	std::remove( ( XorStr( "kaaba\\cfg\\" ) + configname + XorStr( ".kab" ) ).c_str( ) );
}
...
void ConfigManager::OpenConfigFolder( ) {
	...
	std::wstring str = full_path.wstring( ) + XorStr( L"\\kaaba\\cfg" );
