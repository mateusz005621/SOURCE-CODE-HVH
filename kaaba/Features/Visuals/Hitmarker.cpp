...
void Hitmarker::PlayerHurt( IGameEvent* pEvent ) {
	...
	int attacker = g_pEngine->GetPlayerForUserID( pEvent->GetInt( XorStr( "attacker" ) ) );
	int player = g_pEngine->GetPlayerForUserID( pEvent->GetInt( XorStr( "userid" ) ) );

	if( attacker != g_pEngine->GetLocalPlayer( ) || player == g_pEngine->GetLocalPlayer( ) )
		return;
