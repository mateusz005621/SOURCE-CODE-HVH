...
void BeamEffects::Draw( ) {
	...
	for( size_t i{ 0 }; i < m_vecBeamEffects.size( ); i++ ) {
		...
		if( std::abs( g_pGlobalVars->realtime - currentImpact.m_flExpTime ) > 3.f ) {
		...
		if( currentImpact.m_bRing ) {
			...
			beamInfo.m_nModelIndex = g_pModelInfo->GetModelIndex( XorStr( "sprites/purplelaser1.vmt" ) );
			beamInfo.m_pszHaloName = XorStr( "sprites/purplelaser1.vmt" );
			beamInfo.m_nHaloIndex = g_pModelInfo->GetModelIndex( XorStr( "sprites/purplelaser1.vmt" ) );
			...
			auto beam = g_pRenderBeams->CreateBeamRingPoint( beamInfo );
			if( beam ) {
		...
		if( bFinalImpact || currentImpact.m_nIndex != pLocal->EntIndex( ) ) {
			...
			const auto beam = g_pRenderBeams->CreateBeamPoints( beamInfo );
