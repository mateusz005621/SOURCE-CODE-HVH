...
namespace Hooked
{
	void m_angEyeAngles( const CRecvProxyData* pData, void* pStruct, void* pOut ) {
		...
		if( player ) {
			Math::PitchNormalize( pData->m_Value.m_Float );
			Math::Clamp( pData->m_Value.m_Float, -90.f, 90.f );

			m_angEyeAnglesSwap->GetOriginalFunction( )( pData, pStruct, pOut );
		}
	}
		...
		if( pStruct ) {
			...
			if( ent->EntIndex( ) == g_pEngine->GetLocalPlayer( ) ) {
				/*if( pData->m_Value.m_Float < g_Vars.globals.LastVelocityModifier ) {
					...
					*( uint8_t* )( uintptr_t( g_pPrediction.Xor( ) ) + 0x24 ) = 1; // m_bPreviousAckHadErrors 
				}*/
			}
		}
	}
