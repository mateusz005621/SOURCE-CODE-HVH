...
void KitParser::initialize_kits( ) {
	...
	g_KitParser.vecWeaponInfo = {
	};

	vecWeaponInfo[ WEAPON_KNIFE ] = { ( "models/weapons/v_knife_default_ct.mdl" ), ( "knife_default_ct" ) };
	vecWeaponInfo[ WEAPON_KNIFE_T ] = { ( "models/weapons/v_knife_default_t.mdl" ), ( "knife_default_t" ) };

	g_KitParser.vecKnifeNames = {

	};

	g_KitParser.vecGloveNames = {
		//{ 0, ( "Default" ) },
...
#ifndef DEV
	const auto sig_address = Memory::GetAddress( loader_hash( XorStr( "E8 ? ? ? ? FF 76 0C 8D 48 04 E8" ) ) );
#else
	...
	// Dump paint kits
	{
		...
		for( auto i = 0; i <= items_head->last_element; ++i ) {
			...
			if( item->item_sub_position >= LOADOUT_POSITION_MELEE && item->item_sub_position <= LOADOUT_POSITION_HEAVY5 || item->item_sub_position == LOADOUT_POSITION_SPACER4 ) {
				...
				def.world_model = item->GetWorldDisplayModel( );
				items[ node.key ] = def;
			}
		}

	#ifndef DEV
		const auto icons_head = reinterpret_cast< Head_t< int, AlternateIconData_t >* >( std::uintptr_t( item_schema ) + *( int * )( Memory::GetAddress( loader_hash( XorStr( "8D 9A ?? ?? ?? ?? 89 45 9C" ) ) ) + 2 ) + 4 );
	#else
		...
		vecPaintKits.reserve( map_head->allocation_count );

		for( auto i = 0; i <= map_head->last_element; ++i ) {
			const auto paintKit = map_head->memory[ i ].value;

			if( paintKit->id == 9001 )
				continue;

			const auto wide_name = g_pLocalize->Find( paintKit->item_name.buffer + 1 );
			char name[ 256 ];
			V_UCS2ToUTF8( wide_name, name, sizeof( name ) );

			all_skins.push_back( { paintKit->id, name, paintKit->name.buffer } );

			if( paintKit->id < 10000 ) {
				vecPaintKits.push_back( { paintKit->id, name, paintKit->name.buffer } );
			}
			else {
				vecGlovePaintKits.push_back( { paintKit->id, name, paintKit->name.buffer } );
			}
		}

	#ifndef DEV
		const auto collections = reinterpret_cast< Head_t< int, CEconItemSetDefinition >* >( std::uintptr_t( item_schema ) + *( int * )( Memory::GetAddress( loader_hash( XorStr( "74 43 8B 81 ?? ?? ?? ?? 89 45 FC" ) ) ) + 4 ) );
	#else
		...
		for( auto i = 0; i <= collections->last_element; ++i ) {
			...
			for( int i = 0; i < collect.items.m_Size; ++i ) {
				...
				auto skin_it = std::find( vecPaintKits.begin( ), vecPaintKits.end( ), ( entry.paintkit ) );
				if( skin_it == vecPaintKits.end( ) )
					continue;

				auto tmp = *skin_it;

				auto new_item = std::find( vecWeapons.begin( ), vecWeapons.end( ), ( entry.item ) );
				if( new_item == vecWeapons.end( ) ) {
					new_item = vecWeapons.insert( vecWeapons.end( ), item_skins{ } );
				}

				new_item->m_kits.push_back( tmp );
				new_item->display_name = items[ entry.item ].display_name;
				new_item->world_model = items[ entry.item ].world_model;
		...
		std::sort( vecWeapons.begin( ), vecWeapons.end( ), [ ] ( const item_skins &a, const item_skins &b ) {
		...
		for( auto &info : items ) {
			...
			auto new_item = std::find( vecWeapons.begin( ), vecWeapons.end( ), info.first );
			if( new_item == vecWeapons.end( ) ) {
				new_item = vecWeapons.insert( vecWeapons.end( ), item_skins{ } );
			}
			...
			if( items[ size_t( info.first ) ].glove || items[ size_t( info.first ) ].knife ) {
				for( int i = 0; i < icons_head->last_element; ++i ) {
					...
					if( !items[ size_t( info.first ) ].glove ) {
						auto skin_it = std::find_if( vecPaintKits.begin( ), vecPaintKits.end( ), [ skin_str ] ( const paint_kit &a ) {
						...
						auto skin_it = std::find_if( vecGlovePaintKits.begin( ), vecGlovePaintKits.end( ), [ skin_str ] ( const paint_kit &a ) {
							return strcmp( skin_str.c_str( ), a.item_name.data( ) ) == 0;
						} );

						if( skin_it == vecGlovePaintKits.end( ) )
							continue;

						new_item->m_kits.push_back( *skin_it );
						vecGlovePaintKits.erase( skin_it );
					}
				}
			}
			...
			new_item->world_model = items[ size_t( info.first ) ].world_model;
			new_item->id = info.first;

			const auto wide_name = g_pLocalize->Find( new_item->display_name.c_str( ) );
			char buffer[ 256 ];
			...
			if( items[ size_t( info.first ) ].glove || items[ size_t( info.first ) ].knife ) {
				if( new_item->glove ) {
					vecGloveNames.push_back( WeaponName_t( new_item->id, new_item->display_name.c_str( ) ) );
					vecWeaponInfo[ new_item->id ].icon = nullptr;
				}
				else {
					...
					vecKnifeNames.push_back( WeaponName_t( new_item->id, new_item->display_name.c_str( ) ) );
					vecWeaponInfo[ new_item->id ].icon = meme;
				}

				vecWeaponInfo[ new_item->id ].model = new_item->model_name.c_str( );
				vecWeaponInfo[ new_item->id ].world_model = new_item->world_model.c_str( );
			}
		}


		for( auto &value : vecWeapons ) {
			std::sort( value.m_kits.begin( ), value.m_kits.end( ) );
		}

		// copy array contents
		if( !vecPaintKits.empty( ) ) {
			std::sort( vecPaintKits.begin( ), vecPaintKits.end( ) );
			for( auto i = 0; i < vecPaintKits.size( ); ++i ) {
				g_Vars.globals.m_vecPaintKits.emplace_back( vecPaintKits[ i ].name );
			}
		}
	}
