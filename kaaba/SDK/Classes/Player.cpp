...
float C_BasePlayer::GetLayerSequenceCycleRate(C_AnimationLayer* pLayer, int iSequence) {
	using Fn = float(__thiscall*)(void*, C_AnimationLayer*, int);
	return Memory::VCall<Fn>(this, 222)(this, pLayer, iSequence);
}
...
Vector C_CSPlayer::GetHitboxPosition(int hitbox) {
	matrix3x4_t* pMatrix = this->m_CachedBoneData().Base();
	auto pModel = this->GetModel();
	if (!pModel)
		return Vector();
	auto pHdr = g_pModelInfo->GetStudiomodel(pModel);
	if (!pHdr)
		return Vector();
	auto pHitboxSet = pHdr->pHitboxSet(this->m_nHitboxSet());
	if (!pHitboxSet)
		return Vector();
	if (hitbox < 0 || hitbox >= pHitboxSet->numhitboxes)
		return Vector();
	auto pHitbox = pHitboxSet->pHitbox(hitbox);
	if (!pHitbox)
		return Vector();
	return (pHitbox->bbmin + pHitbox->bbmax).Transform(pMatrix[pHitbox->bone]) * 0.5f;
}