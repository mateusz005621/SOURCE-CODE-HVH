...
void GameEvent::FireGameEvent( IGameEvent *pEvent ) {
	...
	auto HitgroupToString = [ ] ( int hitgroup ) -> std::string {
		switch( hitgroup ) {
			case Hitgroup_Generic:
				return g_Vars.menu.polish_mode ? XorStr( "chuja" ) : XorStr( "generic" );
			case Hitgroup_Head:
				return g_Vars.menu.polish_mode ? XorStr( "glowe" ) : XorStr( "head" );
			case Hitgroup_Chest:
				return g_Vars.menu.polish_mode ? XorStr( "klatke piersiowa" ) : XorStr( "chest" );
			case Hitgroup_Stomach:
				return g_Vars.menu.polish_mode ? XorStr( "brzuch" ) : XorStr( "stomach" );
			case Hitgroup_LeftArm:
				return g_Vars.menu.polish_mode ? XorStr( "lewa reke" ) : XorStr( "left arm" );
			case Hitgroup_RightArm:
				return g_Vars.menu.polish_mode ? XorStr( "prawa reke" ) : XorStr( "right arm" );
			case Hitgroup_LeftLeg:
				return g_Vars.menu.polish_mode ? XorStr( "lewa noge" ) : XorStr( "left leg" );
			case Hitgroup_RightLeg:
				return g_Vars.menu.polish_mode ? XorStr( "prawa noge" ) : XorStr( "right leg" );
			case Hitgroup_Neck:
				return g_Vars.menu.polish_mode ? XorStr( "szyje" ) : XorStr( "neck" );
		}
		return XorStr( "cale cialo" );
	};
	...
	switch( event_hash ) {
		...
		case hash_32_fnv1a_const( "bullet_impact" ):
		{
			...
			if( user_index == g_pEngine->GetLocalPlayer( ) ) {
				if( g_Vars.esp.server_impacts ) {
					g_pDebugOverlay->AddBoxOverlay( vecImpact, { -2, -2, -2 }, { 2, 2, 2 }, { }, 0, 0, 255, 125, sv_showimpacts_time->GetFloat( ) );
				}
			}

			auto pEntity = ( C_CSPlayer * )g_pEntityList->GetClientEntity( user_index );
			if( pEntity && !pEntity->IsTeammate( pLocal, false ) ) {

				if( g_Visuals.player_fading_alpha.at( user_index ) > 0.4f ) {
					if( g_Vars.esp.bullet_beams )
						g_BeamEffects.Add( { g_pGlobalVars->realtime, pEntity->GetEyePosition( ), vecImpact, g_Vars.esp.bullet_beams_color.ToRegularColor( ), pEntity->EntIndex( ), pEntity->m_nTickBase( ), false } );
				}
			}
		...
		case hash_32_fnv1a_const( "player_death" ):
		{
			...
			g_Resolver.m_arrResolverData.at( enemy_index ).Reset( );

			//if( g_pEngine->GetPlayerForUserID( pEvent->GetInt( XorStr( "attacker" ) ) ) == g_pEngine->GetLocalPlayer( ) ) {
		...
		case hash_32_fnv1a_const( "item_purchase" ):
		{
			...
			msg << name << ( g_Vars.menu.polish_mode ? XorStr( " kupil " ) : XorStr( " bought " ) ) << item;

			g_EventLog.PushEvent( XorStr( "" ), msg.str( ), Color( 220, 220, 220, 255 ), Color( 220, 220, 220, 255 ) );
			break;
		}
		case hash_32_fnv1a_const( "player_hurt" ):
		{
			...
			auto pEntity = ( C_CSPlayer * )g_pEntityList->GetClientEntity( enemy_index );

			if( pEntity && !pEntity->IsTeammate( pLocal, false ) && g_Vars.esp.visualize_sounds && !pEntity->IsDormant( ) ) {
				g_BeamEffects.Add( { g_pGlobalVars->realtime, pEntity->GetAbsOrigin( ), { }, g_Vars.esp.visualize_sounds_color.ToRegularColor( ), pEntity->EntIndex( ), pEntity->m_nTickBase( ), false, true } );
			}
				...
				if( attacker_index != g_pEngine->GetLocalPlayer( ) ) {
					if( enemy_index == pLocal->m_entIndex ) {
						if( g_Vars.esp.event_harm ) {
							...
							std::string name = attacker_info.szName;
							if( name.length( ) > 15 ) {
								name.resize( 15 );
								name.append( XorStr( "..." ) );
							}

							msg << ( g_Vars.menu.polish_mode ? XorStr( "Obrazony przez " ) : XorStr( "Harmed by " ) ) << name;
							msg << ( g_Vars.menu.polish_mode ? XorStr( " w " ) : XorStr( " in the " ) ) << HitgroupToString( hitgroup ).data( );
							msg << ( g_Vars.menu.polish_mode ? XorStr( " za " ) : XorStr( " for " ) ) << dmg_to_health;
							msg << ( g_Vars.menu.polish_mode ? XorStr( " punktow zdrowia" ) : XorStr( " damage" ) );
							//msg << XorStr( " damage (" ) << remaining_health << XorStr( " health remaining)" );

							g_EventLog.PushEvent( XorStr( "" ), msg.str( ), Color( 220, 220, 220, 255 ), Color( 220, 220, 220, 255 ), 5.f, false );
						}
					}
				}
				else {
					if( g_Vars.esp.event_dmg ) {
						...
						msg << ( g_Vars.menu.polish_mode ? XorStr( "Udezyles " ) : XorStr( "Hit " ) ) << name;
						msg << ( g_Vars.menu.polish_mode ? XorStr( " w " ) : XorStr( " in the " ) ) << HitgroupToString( hitgroup ).data( );
						msg << ( g_Vars.menu.polish_mode ? XorStr( " za " ) : XorStr( " for " ) ) << dmg_to_health;

						if( !g_Vars.menu.polish_mode )
							msg << XorStr( " damage (" ) << remaining_health << XorStr( " health remaining)" );
						else
							msg << XorStr( " punktow zdrowia (pozostalo " ) << remaining_health << XorStr( " pukntow zdrowia)" );

						g_EventLog.PushEvent( XorStr( "" ), msg.str( ), Color( 220, 220, 220, 255 ), Color( 220, 220, 220, 255 ) );
					}

					//if( hitgroup == Hitgroup_Head )
					//	if( g_Resolver.m_arrResolverData.at( enemy_index ).m_bInMoveBodyStage ) {
					//		g_Resolver.m_arrResolverData.at( enemy_index ).m_nPrevHitMoveBodyShots++;
					//	}
					...
					if( g_Vars.misc.hitsound ) {
						if( g_Vars.misc.hitsound_type && !g_Vars.globals.m_vecHitsounds.empty( ) ) {
							...
							g_pSurface->PlaySound_( XorStr( "buttons\\arena_switch_press_02.wav" ) );
						}
					}
				}
			}
		...
		case hash_32_fnv1a_const( "grenade_thrown" ):
			...
			g_GrenadeWarning.m_vecEventNades.push_back( { nThrowerIndex, g_pGlobalVars->curtime } );
			//printf( "grenade_thrown: index %i, spawntime %.6f\n", nThrowerIndex, g_pGlobalVars->curtime );
