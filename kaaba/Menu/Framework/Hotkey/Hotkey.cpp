...
void GUI::Controls::Hotkey( const std::string& name, hotkey_t* var_name, bool type_selection ) {
	...
	if( ctx->FocusedID == 0 ) {
		...
		ctx->DropdownInfo.Elements = { g_Vars.menu.polish_mode ? XorStr( "Przytrzymanie" ) : XorStr( "On hotkey" ),
			g_Vars.menu.polish_mode ? XorStr( "Przelacznik" ) : XorStr( "Toggle" ),
			g_Vars.menu.polish_mode ? XorStr( "Wyl przytrzymanie" ) : XorStr( "Off hotkey" ), 
			g_Vars.menu.polish_mode ? XorStr( "Wlaczone" ) : XorStr( "Always on" ) };
