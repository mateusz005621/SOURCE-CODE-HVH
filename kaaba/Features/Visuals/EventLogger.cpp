...
void EventLogger::Main( ) {
	...
	for( size_t i{ }; i < m_notify_text.size( ); ++i ) {
		...
		Render::Engine::console.string( x, y, Color::White( ).OverrideAlpha( notify->m_width ), notify->m_text );
		y += size;
	}
}

void EventLogger::PushEvent( std::string prefix, std::string msg, Color prefix_color, Color message_color, float time, bool visualise ) {
	if( visualise ) {
		m_notify_text.push_back( std::make_shared< NotifyText >( msg, message_color, time ) );
	}

	g_pCVar->ConsoleColorPrintf( Color( 150, 200, 60 ), XorStr( "[kaaba] " ) );
	if( prefix.size( ) )
		g_pCVar->ConsoleColorPrintf( prefix_color, std::string( prefix + XorStr( " " ) ).c_str( ) );

	g_pCVar->ConsoleColorPrintf( message_color, std::string( msg + XorStr( "\n" ) ).c_str( ) );
}
