...
const auto RoundToNearest = [ & ] ( float in, float multiple ) {
	const auto ratio = static_cast< double >( in ) / multiple;
	const auto iratio = std::lround( ratio );
	return static_cast< float >( iratio * multiple );
};
...
bool GUI::Controls::Slider( const std::string& name, float* var_name, float min, float max, const std::string& display, float increment, bool precision ) {
	...
	if( GUI::SplitStr( name, '#' )[ 0 ].size( ) ) {
		Render::DirectX::Fonts::menu.draw_text( GUI::SplitStr( name, '#' )[ 0 ].data( ), DrawPos + Vector2D( 0, -5 ), ctx->disabled ? Color( 80, 80, 80 ) : Color( 203, 203, 203 ), 4 );

		DrawPos.y += 11;
	}

	bool hovered = g_InputSystem.IsInBox( DrawPos, DrawSize + Vector2D( 0, 1 ) ) && g_InputSystem.IsInBox( ctx->ParentPos, ctx->ParentSize ) && !ctx->disabled;
	...
	if( hovered && ctx->FocusedID != GUI::Hash( name ) )
		...
		if( !ctx->disabled )
			...
			Render::DirectX::gradient_v( DrawPos + 1, DrawSize - 1, { 22, 22, 22 }, { 38, 38, 38 } );
	}
	...
	if( !ctx->disabled )
		...
		Render::DirectX::gradient_v( DrawPos + 1, Vector2D( width + 1, DrawSize.y ) - 1, g_Vars.menu.ascent.ToRegularColor( ) * 0.75f, g_Vars.menu.ascent.ToRegularColor( ) * 0.5f );
	...
	if( ctx->FocusedID == 0 ) {
		...
		if( InputHelper::Down( VK_LBUTTON ) ) {
			...
			if( increment > 1.f ) {
				*var_name = RoundToNearest( GUI::MapNumber( offset, 0, DrawSize.x, min, max ), increment );
			}
			else *var_name = GUI::MapNumber( offset, 0, DrawSize.x, min, max );
...
bool GUI::Controls::Slider( const std::string& name, int* var_name, int min, int max, const std::string& display, int increment, bool precision ) {
	...
	if( ctx->FocusedID == 0 ) {
		...
		if( InputHelper::Down( VK_LBUTTON ) ) {
			...
			if( increment > 1 ) {
				*var_name = std::floor<int>( RoundToNearest( GUI::MapNumber( offset, 0, DrawSize.x, min, max ), increment ) );
			}
			else*var_name = ( GUI::MapNumber( offset, 0, DrawSize.x, min, max ) );
		}
		else {
