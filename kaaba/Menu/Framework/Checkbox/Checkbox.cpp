...
bool GUI::Controls::Checkbox( const std::string& name, bool* value, bool unsafe ) {
	...
	Render::DirectX::Fonts::menu.draw_text( GUI::SplitStr( name, '#' )[ 0 ].data( ), DrawPos + Vector2D( 20, -3 ),
		ctx->disabled ? Color( 80, 80, 80 ) : ( unsafe ? Color( 180, 180, 100 ) : Color( 200, 200, 200 ) ), 0 );

	bool hovered = g_InputSystem.IsInBox( DrawPos, DrawSize - 1 ) && g_InputSystem.IsInBox( ctx->ParentPos, ctx->ParentSize ) && !ctx->disabled;
	bool hovered_text = g_InputSystem.IsInBox( DrawPos + Vector2D( 0, -3 ), Render::DirectX::Fonts::menu.measure_size( GUI::SplitStr( name, '#' )[ 0 ].data( ) ) + Vector2D( 20, 0 ) ) && g_InputSystem.IsInBox( ctx->ParentPos, ctx->ParentSize ) && !ctx->disabled;

	if( !( *value ) ) {
		if( ctx->disabled ) {
			Render::DirectX::gradient_v( DrawPos + 1, DrawSize - 2, Color( 39, 39, 39 ), Color( 30, 30, 30 ) );
		}
		else {
