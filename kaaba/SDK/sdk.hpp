...
#define TICKS_TO_TIME(t) (g_pGlobalVars->interval_per_tick * float(t) )
#define ROUND_TO_TICKS( t ) ( g_pGlobalVars->interval_per_tick * TIME_TO_TICKS( t ) )
...
// note - maxwell; found this in source leak...
...
// https://github.com/obama-gaming/xor-float
#define XorFlt(n) ::XorFloat::Convert(n)
namespace XorFloat {
	...
	constexpr uint32_t XorKey = ::XorFloat::GenerateKey( );
	__forceinline float ConvertBack( const uint32_t val ) {
		const auto xor_key_m128 = _mm_castsi128_ps( _mm_cvtsi32_si128( ::XorFloat::XorKey ) );
	...
	__forceinline float Convert( float val ) {
		...
		cache ^= ::XorFloat::XorKey;
		return XorFloat::ConvertBack( cache );
	}
}
...
#endif
#define MDLCACHE_CRITICAL_SECTION() MDLCACHE_CRITICAL_SECTION_(g_pMDLCache.Xor()) // Your interface ptr
#define MDLCACHE_COARSE_LOCK() MDLCACHE_COARSE_LOCK_(g_pMDLCache.Xor()) // Your interface ptr
