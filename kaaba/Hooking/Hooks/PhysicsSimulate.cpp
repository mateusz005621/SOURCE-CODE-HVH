...
void __fastcall Hooked::PhysicsSimulate( void* ecx, void* edx ) {
	...
	if( !pCommandContext || g_pGlobalVars->tickcount == nSimulationTick || !pCommandContext->needsprocessing ) {
		return oPhysicsSimulate( ecx );
	}
	...
	if( g_Prediction.m_nLastFrameStage == 4 ) {
		g_Prediction.m_nLastTickCount = pCommandContext->cmd.tick_count;
	}
}
