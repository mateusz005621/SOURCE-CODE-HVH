...
void line( Vector2D pos, bool nigger = false ) {
	for( auto i = 5; i >= 2; --i ) {
		auto offset = 5 - i;
		Render::DirectX::line( Vector2D( pos.x + offset, pos.y + offset ), Vector2D( pos.x + offset + std::clamp( i - offset, 0, 5 ), pos.y + offset ), nigger ? Color( 0, 0, 0 ) : (
			GUI::ctx->disabled ? Color( 68, 68, 68 ) : Color( 151, 151, 151 ) ) );
	}
}
...
bool GUI::Controls::Dropdown( const std::string& name, std::vector< std::string > options, int* var_name, int max_items ) {
	...
	if( !GUI::SplitStr( name, '#' )[ 0 ].empty( ) ) {
		Render::DirectX::Fonts::menu.draw_text( GUI::SplitStr( name, '#' )[ 0 ].data( ), DrawPos + Vector2D( 0, -3 ), ctx->disabled ? Color( 80, 80, 80 ) : Color( 203, 203, 203 ), 4 );
	...
	bool hovered = g_InputSystem.IsInBox( DrawPos, DrawSize ) && g_InputSystem.IsInBox( ctx->ParentPos, ctx->ParentSize ) && !ctx->disabled;
	...
	if( hovered || ctx->DropdownInfo.HashedID == GUI::Hash( name ) )
		...
		if( !ctx->disabled )
			...
			Render::DirectX::gradient_v( DrawPos, DrawSize, { 21, 21, 21 }, { 23, 23, 23 } );
	}
	...
	Render::DirectX::Fonts::menu.draw_text( options[ *var_name ], DrawPos + Vector2D( 10, 4 ), ctx->disabled ? Color( 80, 80, 80 ) : Color{ 150, 150, 150 }, 0 );
