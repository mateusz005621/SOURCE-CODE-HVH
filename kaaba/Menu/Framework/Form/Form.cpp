...
bool GUI::Form::BeginWindow( std::string name ) {
	...
	auto rainbow = [ ] ( Vector2D pos, bool dark ) {
		if( g_Vars.menu.polish_mode ) {
			// Render::DirectX::rect_fill( pos + 1, Vector2D( ctx->size.x, 1 ), dark ? Color(255,0,0):Color( 255, 255, 255 ) );
			Render::DirectX::gradient_h( pos + 1, Vector2D( ctx->size.x / 2, 1 ), Color( 255, 255, 255 ), Color( 220, 127, 127 ) );
			Render::DirectX::gradient_h( Vector2D( pos.x + ctx->size.x / 2, pos.y + 1 ), Vector2D( ( ctx->size.x / 2 ) - 1, 1 ), Color( 220, 127, 127 ), Color( 255, 0, 0 ) );
...
void GUI::Form::EndWindow( ) {
	...
	if( ctx->ColorPickerInfo.HashedID != 0 && ctx->animation >= 1.0f ) {
		...
		if( ctx->ColorPickerInfo.ActionCringe == 1 ) {
			...
			Render::DirectX::Fonts::menu.draw_text( g_Vars.menu.polish_mode ? XorStr( "Kopiuj" ) : XorStr( "Copy" ), DrawPos + Vector2D( 10, 4 ), Color( 200, 200, 200 ), 0 );
			Render::DirectX::Fonts::menu.draw_text( g_Vars.menu.polish_mode ? XorStr( "Wklej" ) : XorStr( "Paste" ), PastePos + Vector2D( 10, 4 ), Color( 200, 200, 200 ), 0 );
