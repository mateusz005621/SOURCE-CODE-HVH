...
void __fastcall Hooked::RunCommand( void* ecx, void* edx, C_CSPlayer* player, CUserCmd* ucmd, IMoveHelper* moveHelper ) {
	...
	// out of range?
	if( !( g_pGlobalVars->tickcount + 64 + g_Vars.sv_max_usercmd_future_ticks->GetInt( ) > ucmd->tick_count ) ) {
	...
	if( g_TickbaseController.m_bFixCharge && !g_TickbaseController.m_bFixedCharge ) {
		...
		//	player->m_nTickBase( ) += 1;
		//}

		g_pGlobalVars->curtime = float( player->m_nTickBase( ) ) * g_pGlobalVars->interval_per_tick;
	...
	// whatever, fucked on 0 ping, fine on ping
	if( g_TickbaseController.m_bPostFix && ( g_TickbaseController.m_iFixCommand + 1 + g_pClientState->m_nChokedCommands( ) ) == ucmd->command_number && g_TickbaseController.m_iFixAmount != 0 ) {
	...
	auto FixPostponeTime = [ player ] ( int command_number ) {
		...
		if( weapon ) {
			...
			if( weapon->m_iItemDefinitionIndex( ) == WEAPON_REVOLVER ) {
				auto tick_rate = int( 1.0f / g_pGlobalVars->interval_per_tick );
				if( tick_rate >> 1 > 1 ) {
	...
	g_Prediction.StoreCompressionNetvars( );
