...
void C_BaseEntity::DrawHitboxMatrix( matrix3x4_t* bones, mstudiobbox_t* pHitbox ) {
	...
	auto RenderCapsule = [ & ] ( mstudiobbox_t* hitbox ) {
		...
		g_pDebugOverlay->AddCapsuleOverlay( min, max, hitbox->m_flRadius,
			g_Vars.esp.target_capsules_color.r * 255.f,
			g_Vars.esp.target_capsules_color.g * 255.f,
			g_Vars.esp.target_capsules_color.b * 255.f,
			g_Vars.esp.target_capsules_color.a * 255.f, g_pCVar->FindVar( XorStr( "sv_showlagcompensation_duration" ) )->GetFloat( ) );
	};
