...
void Render::DirectX::create_objects( ) {
	if( FAILED( device->CreateStateBlock( D3DSBT_PIXELSTATE, &m_stateblock ) ) ) {
		m_candraw = false;
	}
	else {
		m_candraw = true;
	...
	D3DXCreateFontA( device,
					 Fonts::menu.height, 0,
					 Fonts::menu.weight, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
					 Fonts::menu.anti_aliased ? CLEARTYPE_NATURAL_QUALITY : NONANTIALIASED_QUALITY, DEFAULT_PITCH,
					 Fonts::menu.name.data( ), &Fonts::menu.p_font );

	D3DXCreateFontA( device,
					 Fonts::menu_pixel.height, 0,
					 Fonts::menu_pixel.weight, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
					 Fonts::menu_pixel.anti_aliased ? CLEARTYPE_NATURAL_QUALITY : NONANTIALIASED_QUALITY, DEFAULT_PITCH,
					 Fonts::menu_pixel.name.data( ), &Fonts::menu_pixel.p_font );


	D3DXCreateFontA( device,
					 Fonts::menu_bold.height, 0,
					 Fonts::menu_bold.weight, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
					 Fonts::menu_bold.anti_aliased ? CLEARTYPE_NATURAL_QUALITY : NONANTIALIASED_QUALITY, DEFAULT_PITCH,
					 Fonts::menu_bold.name.data( ), &Fonts::menu_bold.p_font );

	D3DXCreateFontA( device,
					 Fonts::menu_icon.height, 0,
					 Fonts::menu_icon.weight, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
					 Fonts::menu_icon.anti_aliased ? CLEARTYPE_NATURAL_QUALITY : NONANTIALIASED_QUALITY, DEFAULT_PITCH,
					 Fonts::menu_icon.name.data( ), &Fonts::menu_icon.p_font );

	Textures::background->setup( device, background_texture, sizeof( background_texture ), 2000, 2000 );
}

void Render::DirectX::initialize( IDirect3DDevice9 *dev ) {
	device = dev;
	create_objects( );
}

void Render::DirectX::release_objects( ) {
	if( !m_candraw ) {
		return;
	}
	...
	if( Fonts::menu.p_font ) {
		Fonts::menu.p_font->Release( );
	}

	if( Fonts::menu_pixel.p_font ) {
		Fonts::menu_pixel.p_font->Release( );
	}

	if( Fonts::menu_bold.p_font ) {
		Fonts::menu_bold.p_font->Release( );
	}

	if( Fonts::menu_icon.p_font ) {
		Fonts::menu_icon.p_font->Release( );
	}
...
void Render::DirectX::circle_filled( Vector2D center, float radius, int rotate, int type, bool smoothing, int resolution, Color c ) {
	...
	device->CreateVertexBuffer( ( resolution + 2 ) * sizeof( vertice ), D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW | D3DFVF_DIFFUSE, D3DPOOL_DEFAULT, &m_vb, NULL );

	void *pVertices;
	m_vb->Lock( 0, ( resolution + 2 ) * sizeof( vertice ), ( void ** )&pVertices, 0 );
	...
	device->SetStreamSource( 0, m_vb, 0, sizeof( vertice ) );
	device->SetFVF( D3DFVF_XYZRHW | D3DFVF_DIFFUSE );
...
void Render::Engine::Initialise( ) {
	...
	g_pEngine->GetScreenSize( m_width, m_height );
...
void Render::Engine::RoundedRect( const Vector2D &pos, const Vector2D &size, const int radius, const int corners, const Color color ) {
	...
	//g_pSurface->DrawSetTextureRGBA( texture, Color::White( ).RGBA, 1, 1 );
...
// disgusting im sorry
void Render::Engine::Arc( Vector2D pos, float radius, float start_angle, float percent, float thickness, Color color ) {
	const auto precision = ( 2 * M_PI ) / 30;
	const auto step = M_PI / 180;
	...
	for( ; radius > inner; --radius ) {
		for( auto angle = start_angles; angle < end_angle; angle += precision ) {
			auto center_x = std::round( pos.x + radius * std::cos( angle ) );
			auto center_y = std::round( pos.y + radius * std::sin( angle ) );

			auto center_x2 = std::round( pos.x + radius * std::cos( angle + precision ) );
			auto center_y2 = std::round( pos.y + radius * std::sin( angle + precision ) );

			Line( center_x, center_y, center_x2, center_y2, color );
		}
	}
}
